...
{
	int dx, dy, angle, mindist, magangle, xydist, dz;
	objtype *temp;
	playertype *pstate;
...
void Cmd_Use(objtype *ob)
{
	int checkx, checky, doorn,
		/*newtilex,newtiley,oldtilex,oldtiley,*/ elevnum, wallx, wally;
	...
	{
		if ((tempactor->dir == nodir) || (tempactor->dir == ob->dir))
		{
...
#if (BNACRASHPREVENT == 1) // crashed here when oscuro and larves were all
						   // killed
		if (obj->next == 0)
		{
			return;
...
#if (SHAREWARE == 0)
	if ((pstate->buttonstate[bt_use]) && (pstate->weapon != wp_dog))
#else
